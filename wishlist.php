    ...
    document.getElementById('remove-btn').disabled = checkboxes.length === 0;
        ...
        function addToCart() {
            ...
            if (productIds.length > 0) {
                sendRequest('add_to_cart=true', productIds, quantities);
            }
        }
        ...
        function removeSelectedProducts() {
            ...
            if (productIds.length > 0) {
                sendRequest('remove=true', productIds);
            }
        }
...
function decrementQuantity(productId) {
    ...
    if (input.value > min) {
        ...
        function buyNow() {
            ...
            if (productIds.length > 0) {
                sendRequest('buy_now=true', productIds, quantities);
            }
        }

        function sendRequest(action, productIds, quantities = {}) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'wishlist.php', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            ...
            const payload = action + '&product_ids=' + productIds.join(',') + '&quantities=' + JSON.stringify(quantities);
            xhr.send(payload);
        }
    </script>
</body>
